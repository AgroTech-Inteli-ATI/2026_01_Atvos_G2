...
def calcular_necessidade_calagem(talhao: dict) -> dict:
    ...
    if __name__ == "__main__":
        ...
        casos = [
            # Caso 1: cana planta, Mg baixo -> dolomítico
            {"id_talhao": "T001", "categoria": "Formação",
             "V1": 42.0, "CTC1": 90.0, "mg1": 3.5},
            # Caso 2: cana planta, Mg ok -> calcítico ou dolomítico
            {"id_talhao": "T002", "categoria": "Formação",
             "V1": 50.0, "CTC1": 80.0, "mg1": 7.0},
            # Caso 3: V% já adequado -> sem calagem
            {"id_talhao": "T003", "categoria": "Formação",
             "V1": 65.0, "CTC1": 80.0, "mg1": 6.0},
            # Caso 4: cana soca -> fator 0.5
            {"id_talhao": "T004", "categoria": "Cana Soca",
             "V1": 40.0, "CTC1": 100.0, "mg1": 6.0},
            # Caso 5: V1 ausente -> SEM_DADO
            {"id_talhao": "T005", "V1": None, "CTC1": 80.0, "mg1": 5.0},
            # Caso 6: CTC1 NaN -> SEM_DADO
            {"id_talhao": "T006", "V1": 40.0, "CTC1": float("nan"), "mg1": 5.0},
        ]

        for caso in casos:
            resultado = calcular_necessidade_calagem(caso)
            print(f"\n[{caso.get('id_talhao')}]")
...
def calcular_necessidade_gessagem(talhao: dict) -> dict:
    ...
    if __name__ == "__main__":
        ...
        casos = [
            # Caso 1: Ca baixo -> gessagem por deficiência de Ca
            {"id_talhao": "T001", "categoria": "Formação",
             "ca2": 2.5, "al2": 5.0, "sb2": 15.0, "tipo_solo": "Argiloso"},
            # Caso 2: Sat Al alta -> gessagem por toxidez
            {"id_talhao": "T002", "categoria": "Formação",
             "ca2": 6.0, "al2": 10.0, "sb2": 10.0, "tipo_solo": "Médio"},
            # Caso 3: ambos adequados -> sem gessagem
            {"id_talhao": "T003", "categoria": "Formação",
             "ca2": 5.0, "al2": 3.0, "sb2": 20.0, "tipo_solo": "Muito Argiloso"},
            # Caso 4: cana soca -> não aplicável
            {"id_talhao": "T004", "categoria": "Cana Soca",
             "ca2": 1.0, "al2": 10.0, "sb2": 5.0, "tipo_solo": "Arenoso"},
            # Caso 5: solo não mapeado -> fallback "A Definir"
            {"id_talhao": "T005", "categoria": "Formação",
             "ca2": 2.0, "al2": 8.0, "sb2": 12.0, "tipo_solo": "Tipo Desconhecido"},
            # Caso 6: ca2 ausente -> SEM_DADO
            {"id_talhao": "T006", "categoria": "Formação",
             "ca2": None, "al2": 8.0, "sb2": 12.0, "tipo_solo": "Médio"},
        ]

        for caso in casos:
            resultado = calcular_necessidade_gessagem(caso)
            print(f"\n[{caso.get('id_talhao')}]")
            print(json.dumps(resultado, ensure_ascii=False, indent=2))
...
def calcular_necessidade_fosfatagem(talhao: dict) -> dict:
    ...
    if __name__ == "__main__":
        ...
        casos = [
            # Caso 1: P muito baixo -> 120 kg/ha, prioridade alta
            {"id_talhao": "T001", "categoria": "Formação", "p1": 4.5},
            # Caso 2: P baixo -> 80 kg/ha, prioridade média
            {"id_talhao": "T002", "categoria": "Formação", "p1": 9.0},
            # Caso 3: P médio -> 40 kg/ha, prioridade baixa
            {"id_talhao": "T003", "categoria": "Formação", "p1": 18.0},
            # Caso 4: P suficiente -> 0, sem aplicação
            {"id_talhao": "T004", "categoria": "Formação", "p1": 30.0},
            # Caso 5: cana soca -> não aplicável
            {"id_talhao": "T005", "categoria": "Cana Soca", "p1": 4.0},
            # Caso 6: p1 ausente -> SEM_DADO
            {"id_talhao": "T006", "categoria": "Formação", "p1": None},
            # Caso 7: p1 no limite exato de P_MUITO_BAIXO (6.0) -> P baixo
            {"id_talhao": "T007", "categoria": "Formação", "p1": 6.0},
        ]

        for caso in casos:
            resultado = calcular_necessidade_fosfatagem(caso)
            print(f"\n[{caso.get('id_talhao')}]")
            print(json.dumps(resultado, ensure_ascii=False, indent=2))
...
def calcular_erradicacao_soqueira(talhao: dict) -> dict:
    ...
    def calcular_erradicacao_soqueira(talhao: dict) -> dict:
        ...
        for campo in ("tch_prod", "no_corte"):
            if _(talhao.get(campo)):
                return _(f"dado_ausente_{campo}")
    ...
    if __name__ == "__main__":
        ...
        casos = [
            # Caso 1: longevidade máxima -> reforma obrigatória
            {"id_talhao": "T001", "categoria": "Cana Soca",
             "tch_prod": 60.0, "no_corte": 9, "sit_talhao": "Fechado"},
            # Caso 2: TCH baixo, corte maduro -> reforma alta prioridade
            {"id_talhao": "T002", "categoria": "Cana Soca",
             "tch_prod": 48.0, "no_corte": 4, "sit_talhao": "Cana Soca"},
            # Caso 3: TCH baixo, corte jovem -> reforma + investigação
            {"id_talhao": "T003", "categoria": "Cana Soca",
             "tch_prod": 40.0, "no_corte": 2, "sit_talhao": "Cana Soca"},
            # Caso 4: TCH ok, longevidade elevada -> reforma preventiva
            {"id_talhao": "T004", "categoria": "Cana Soca",
             "tch_prod": 70.0, "no_corte": 7, "sit_talhao": "Fechado"},
            # Caso 5: talhão produtivo -> continuar ciclo
            {"id_talhao": "T005", "categoria": "Cana Soca",
             "tch_prod": 75.0, "no_corte": 3, "sit_talhao": "Cana Soca"},
            # Caso 6: cana planta -> não aplicável
            {"id_talhao": "T006", "categoria": "Formação",
             "tch_prod": 0.0, "no_corte": 0, "sit_talhao": "Cana Planta"},
            # Caso 7: tch_prod ausente -> SEM_DADO
            {"id_talhao": "T007", "categoria": "Cana Soca",
             "tch_prod": None, "no_corte": 5, "sit_talhao": "Fechado"},
            # Caso 8: reforma indicada mas situação desconhecida
            {"id_talhao": "T008", "categoria": "Cana Soca",
             "tch_prod": 30.0, "no_corte": 5, "sit_talhao": "Em Colheita"},
        ]

        for caso in casos:
            resultado = calcular_erradicacao_soqueira(caso)
            print(f"\n[{caso.get('id_talhao')}]")
            print(json.dumps(resultado, ensure_ascii=False, indent=2))